...
function ollama_model_meta(string $modelId): array {
    $defaults = [
        ...
        'supports_tools' => true, 'context_window' => null, 'context_window_native' => null,
    ];
...
function ollama_chat_stream(array $messages, callable $onToken, ?string $thinkOverride = null, ?string $modelOverride = null): array {
    $baseUrl = rtrim((string)app_config('ollama_base_url'), '/');
...
METODE UMUM UNTUK TUGAS KOMPLEKS
- Identifikasi dulu jenis tugas: fakta terkini, perhitungan, analisis berbasis data, logika, atau rekomendasi. Pisahkan data yang diketahui, asumsi, dan kesimpulan sebelum menjawab.
- Untuk perhitungan bertahap, buat jejak input → rumus → hasil dan gunakan tool; periksa ulang hasil penting dengan satuan, batas, dan relasi yang berbeda. Jangan mengganti hasil tool dengan angka yang dihitung ulang di teks.
- Untuk penalaran/logika, uji kesimpulan terhadap seluruh syarat pengguna dan setidaknya satu alternatif atau kasus tepi yang relevan. Jika data tidak cukup untuk membedakan beberapa kesimpulan, nyatakan batasnya, jangan memilih secara pasti.
- Untuk fakta yang berubah, gunakan sumber terbaru lewat tool. Untuk fakta stabil, jangan membuat sitasi, sumber, atau detail yang tidak Anda miliki.
- Sebelum mengirim jawaban, pastikan setiap klaim utama didukung data pengguna, hasil tool, atau asumsi yang sudah dinyatakan; setiap tabel harus konsisten dengan narasinya.
...
PROMPT;
    ...
    $history = array_map(static fn(array $m): array => ['role' => $m['role'], 'content' => $m['content']], $messages);
    $history = ollama_limit_history(
        $history,
        (int) app_config('history_max_messages'),
        (int) app_config('history_max_chars')
    );
    ...
    foreach ($thinkChain as $thinkLevel) {
        ...
        $attempt = ollama_run_rounds($baseUrl, $model, $ollamaMessages, $tools, $thinkLevel, $onToken, $maxRounds, $deadline, $numCtx);
...
function ollama_run_rounds(string $baseUrl, string $model, array $ollamaMessages, array $tools, $think, callable $onToken, int $maxRounds, ?float $deadline = null, ?int $numCtx = null): array {
    ...
    for ($round = 0; $round < $maxRounds; $round++) {
        ...
        for ($attempt = 1; $attempt <= $attemptsAllowed; $attempt++) {
            ...
            $toolChoice = ($round === 0 && $attempt === 1 && !empty($tools) && tool_calc_looks_calculation_heavy())
                ? 'required' : null;
            $r = ollama_stream_once($baseUrl, $model, $ollamaMessages, $tools, $thinkStr, $onToken, $numCtx, $toolChoice);
            if ($r['ok']) break;
            if ($attempt < $attemptsAllowed) usleep(700000 * $attempt); 
        }
        if (!$r['ok']) return ['ok' => false, 'error' => $r['error'], 'friendly' => $r['friendly'] ?? false];
        $answer .= $r['content'];
            ...
            if (!empty($toolWarnings) && !$warningsPrompted) {
                ...
                $ollamaMessages[] = [
                    ...
                    'content' => "Sebelum benar-benar selesai: hasil tool sebelumnya memuat peringatan berikut. "
                        ...
                        . "Jika sudah tercakup, cukup balas konfirmasi singkat tanpa mengulang.\n\n{$warningList}",
                ];
                continue;
            }
        ...
        foreach ($toolCalls as $call) {
            ...
            $toolResult = $fname !== '' ? ollama_tool_execute($fname, $args) : 'Tool tanpa nama tidak dapat dijalankan.';
    ...
    $final = ollama_stream_once($baseUrl, $model, $ollamaMessages, [], $thinkStr, $onToken, $numCtx);
...
function ollama_stream_once(string $baseUrl, string $model, array $ollamaMessages, array $tools, string $think, callable $onToken, ?int $numCtx = null, ?string $toolChoice = null, bool $lightweight = false, ?array $optionsOverride = null): array {
    $payloadArr = [
        'model' => $model,
        ...
        'options' => [
            ...
            'num_predict' => $lightweight ? 300 : 8000,
        ],
    ];
    if ($think !== '') {
    ...
    if (function_exists('curl_init')) {
        ...
        if ($ok === false && $error === '') {
            ...
            return ollama_build_error_result('Gagal menghubungi Ollama: ' . $curlErr, $httpCode, $retryAfter);
        }
